...
# 8 "./mcc_generated_files/adc.h" 2
# 34 "./mcc_generated_files/adc.h"
...
# 8 "./mcc_generated_files/epwm2.h" 2

# 1 "./mcc_generated_files/tmr0.h" 1
# 37 "./mcc_generated_files/tmr0.h"
...
# 9 "./mcc_generated_files/epwm2.h" 2

# 1 "./mcc_generated_files/pin_manager.h" 1
# 57 "./mcc_generated_files/pin_manager.h"
void PIN_MANAGER_Initialize(void);
# 10 "./mcc_generated_files/epwm2.h" 2
# 30 "./mcc_generated_files/epwm2.h"
    typedef struct {
        uint8_t status;
        uint8_t turn;
...
# 74 "./mcc_generated_files/epwm2.h"
...
# 90 "./mcc_generated_files/epwm2.h"
    float EPWM2_duty_const(uint8_t freq);
# 104 "./mcc_generated_files/epwm2.h"
...
# 1 "./mcc_generated_files/epwm3.h" 1
...
# 8 "./mcc_generated_files/epwm3.h" 2
# 30 "./mcc_generated_files/epwm3.h"
    typedef struct {
        uint8_t status;
        uint8_t turn;
...
# 74 "./mcc_generated_files/epwm3.h"
...
# 90 "./mcc_generated_files/epwm3.h"
    float EPWM3_duty_const(uint8_t freq);
# 104 "./mcc_generated_files/epwm3.h"
...
# 27 "./mcc_generated_files/eusart1.h"
    typedef union {
...
# 1 "./mcc_generated_files/Light_sensor_I2C.h" 1




# 1 "./mcc_generated_files/i2c2_master.h" 1
# 45 "./mcc_generated_files/i2c2_master.h"
void i2c2_init(void);
# 65 "./mcc_generated_files/i2c2_master.h"
uint8_t i2c2_Open(uint8_t slaveAddress, uint8_t address);
# 74 "./mcc_generated_files/i2c2_master.h"
uint8_t i2c2_gets(uint8_t slaveAddress, uint8_t *ptrData, uint8_t length);
# 83 "./mcc_generated_files/i2c2_master.h"
uint8_t i2c2_puts(uint8_t *ptrData, uint8_t length);
# 93 "./mcc_generated_files/i2c2_master.h"
_Bool i2c2_control(uint8_t bit_field);
# 5 "./mcc_generated_files/Light_sensor_I2C.h" 2
# 20 "./mcc_generated_files/Light_sensor_I2C.h"
typedef struct {
    uint8_t I2C_state;
    uint8_t SENS_ERROR;
...
I2C_ERROR i2c_Error;
# 99 "./mcc_generated_files/Light_sensor_I2C.h"
...
# 27 "./mcc_generated_files/memory.h"
...
# 34 "./user.h"
    ...
    typedef struct {
        uint16_t Average_Sensor[3];
        uint16_t Average_Sensor_Total;
    } Sampling_light;

    typedef struct {
        uint16_t azimut_angle;
...
void init_App(void) {
    ...
    syst_Stat.i2c_stat = 0;
    request_pos.azimut_angle = ADC_GetConversion(0x01, 10);


    if (request_pos.azimut_angle <= 38)
        syst_Stat.lim_azimuth = 1;
    else if (request_pos.azimut_angle >= 736)
        syst_Stat.lim_azimuth = 2;
    else
        syst_Stat.lim_azimuth = 0;
...
uint8_t command_execution(void) {
    if (EUSART1_is_rx_ready()) {
        ...
        command_get();
        if (caract_count == 1) {
            return short_commands(command[0]);
        }
    }
    return 0;
}
...
uint8_t short_commands(uint8_t short_comm) {
    ...
    switch (short_comm) {
        case 'e':
            if (syst_Stat.mode == 'm' && syst_Stat.lim_azimuth == 0) {
                azi_ctrl = azi_lim_control(0x01);
                if (azi_ctrl == 4 || azi_ctrl == 9)
                    return azi_ctrl;
                else {
                    if (STATUS_Eng_A.status == 0)
                        printf("$e;");
                    EPWM2_engine_SoftStart(0x01, 10);
                    if (syst_Stat.lim_azimuth == 1)
                        syst_Stat.lim_azimuth = 0;
                    return 'e';
                }
            }
        ...
        case 'o':
            if (syst_Stat.mode == 'm' && syst_Stat.lim_azimuth == 0) {
                azi_ctrl = azi_lim_control(0x02);
                if (azi_ctrl == 4 || azi_ctrl == 9)
                    return azi_ctrl;
                else {
                    if (STATUS_Eng_A.status == 0)
                        printf("$o;");
                    EPWM2_engine_SoftStart(0x02, 10);
                    if (syst_Stat.lim_azimuth == 2)
                        syst_Stat.lim_azimuth = 0;
                    return 'o';
                }
            }
        ...
        case 's':
            if (syst_Stat.mode == 'm') {
                if (STATUS_Eng_V.status == 0)
                    printf("$s;");
                EPWM3_engine_SoftStart(0x02, 5);
                return 's';
            }
            break;
        case 'b':
        ...
        case 'a':
            if (syst_Stat.mode != 'a') {
                syst_Stat.mode = 'a';
                DATAEE_WriteByte(0, syst_Stat.mode);
                printf("$a;");
                return 2;
            }
            break;
...
uint8_t Emergency_stop(void) {
    if ((STATUS_Eng_A.status == 1) || (STATUS_Eng_V.status == 1)) {
...
uint8_t azi_lim_control(uint8_t move) {
    current_pos.azimut_angle = ADC_GetConversion(0x01, 10);
    switch (move) {
        case 0x02:
            if (jump_azi_ctrl() == 9) {
                return 9;
            }
            if (current_pos.azimut_angle < 38) {
                if (STATUS_Eng_A.status == 1) {
                    EPWM2_engine_SoftStop();
                    printf("$f;");
                }
                if (syst_Stat.lim_azimuth == 0)
                    syst_Stat.lim_azimuth = 1;
                return 4;
            }
            break;
        case 0x01:
            if (jump_azi_ctrl() == 9)
                return 9;
            if (current_pos.azimut_angle > 736) {
                if (STATUS_Eng_A.status == 1) {
                    EPWM2_engine_SoftStop();
                    printf("$f;");
                }
