...
# 5 "mcc_generated_files/epwm2.h" 2
...
# 6 "mcc_generated_files/epwm2.h" 2

# 1 "C:\\Program Files (x86)\\Microchip\\xc8\\v2.10\\pic\\include\\c99\\stdbool.h" 1 3
# 7 "mcc_generated_files/epwm2.h" 2
...
# 2 "mcc_generated_files/epwm2.c" 2
# 13 "mcc_generated_files/epwm2.c"
...
void EPWM2_Initialize(uint8_t freq) {
    ...
    ECCP2ASbits.PSS2AC = 0;

    ECCP2ASbits.PSS2BD = 0;
    ...
    EPWM2_engine_config(0x00, 50);

    PWM2CONbits.P2DC = 0x50;
    ...
    TRISAbits.TRISA5 = 0;
    EPWM2_outputs_off();
    STATUS_Eng_A.status = 0;
    STATUS_Eng_A.turn = 0x00;
    return;
}
...
void EPWM2_period(uint8_t freq) {
    ...
    if (freq_lure != freq) {
        if (freq <= 4)
            TMR2_LoadPeriodRegister(((64000000 / 1000) / (4 * 4 * 16)) - 1);
        else
            TMR2_LoadPeriodRegister(((64000000 / 1000) / (freq * 4 * 16)) - 1);
        freq_lure = freq;
...
void EPWM2_engine_config(uint8_t turn, uint8_t duty) {
    float interval = EPWM2_duty_const(0);
...
void EPWM2_engine_SoftStart(uint8_t turn, uint8_t dut_max) {
    if (STATUS_Eng_A.status == 0) {
        ...
        STATUS_Eng_A.status = 2;

        count_EPWM2_SStart = 50;
