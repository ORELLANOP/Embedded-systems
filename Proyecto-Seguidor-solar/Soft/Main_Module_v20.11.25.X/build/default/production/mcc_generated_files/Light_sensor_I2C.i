# 1 "mcc_generated_files/Light_sensor_I2C.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 288 "<built-in>" 3
# 1 "<command line>" 1
...
# 2 "<built-in>" 2
# 1 "mcc_generated_files/Light_sensor_I2C.c" 2
...
# 2 "mcc_generated_files/Light_sensor_I2C.h" 2
...
# 3 "mcc_generated_files/Light_sensor_I2C.h" 2

# 1 "C:\\Program Files (x86)\\Microchip\\xc8\\v2.10\\pic\\include\\c99\\stdbool.h" 1 3
# 4 "mcc_generated_files/Light_sensor_I2C.h" 2
...
# 8 "mcc_generated_files/tmr0.h" 2
...
# 8 "mcc_generated_files/i2c2_master.h" 2
...
# 2 "mcc_generated_files/Light_sensor_I2C.c" 2
# 11 "mcc_generated_files/Light_sensor_I2C.c"
void i2c_BH1750_init(void) {
...
uint8_t i2c_BH1750_open(uint8_t channel) {
    switch (channel) {
        case 1:
            {LATD = ((PORTD & 0xF3) | (0x00));}
            break;
        case 2:
            {LATD = ((PORTD & 0xF3) | (0x0C));}
            break;
        case 3:
            {LATD = ((PORTD & 0xF3) | (0x08));}
            break;
        case 4:
            {LATD = ((PORTD & 0xF3) | (0x04));}
            break;
    }
    if (i2c2_Open(0x46, 0x23) == 1) {
        return 1;
    } else
        return 5;
}
...
uint16_t BH1750_Data_Read(uint8_t Read_meas) {
    uint8_t raw_value[2];
    uint16_t lux_value;
    if (i2c2_gets(Read_meas, raw_value, 2) == 3) {
        i2c_Error.Data_Read_error = 3;
        return 0;
    } else
        i2c_Error.Data_Read_error = 7;
...
uint16_t BH1750_measure(uint8_t channel, uint8_t average) {
    ...
    uint32_t acum = 0;
    do {
        while (i2c_Error.i2c_count_error < 5) {
            if (i2c_BH1750_open(channel) == 1)
                i2c_Error.i2c_count_error++;
            else {
                i2c_Error.I2C_state = 5;
                i2c_Error.i2c_count_error = 0;
                break;
            }
        }
        if (i2c_Error.i2c_count_error >= 5) {
            i2c_Error.I2C_state = 1;
            i2c_Error.SENS_ERROR = channel;
            i2c_Error.i2c_count_error = 0;
            return 0;
        }
        _delay((unsigned long)((20)*(64000000/4000.0)));
        Data_lux = BH1750_Data_Read(0x47);
        if (i2c_Error.Data_Read_error == 3) {
            i2c_Error.I2C_state = 3;
            i2c_Error.SENS_ERROR = channel;
            return 0;
        } else {
            acum += Data_lux;
            i++;
            i2c_Error.I2C_state = 7;
        }
    } while (i < average);
    i2c_Error.I2C_state = 0;
    i2c_Error.SENS_ERROR = 0;
