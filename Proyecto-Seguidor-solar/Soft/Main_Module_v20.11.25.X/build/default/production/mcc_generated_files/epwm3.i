...
# 2 "<built-in>" 2
# 1 "mcc_generated_files/epwm3.c" 2
...
# 5 "mcc_generated_files/epwm3.h" 2
...
# 6 "mcc_generated_files/epwm3.h" 2

# 1 "C:\\Program Files (x86)\\Microchip\\xc8\\v2.10\\pic\\include\\c99\\stdbool.h" 1 3
# 7 "mcc_generated_files/epwm3.h" 2
...
# 9 "mcc_generated_files/epwm3.h" 2

# 1 "mcc_generated_files/pin_manager.h" 1
# 57 "mcc_generated_files/pin_manager.h"
void PIN_MANAGER_Initialize(void);
# 10 "mcc_generated_files/epwm3.h" 2
...
# 2 "mcc_generated_files/epwm3.c" 2
# 13 "mcc_generated_files/epwm3.c"
...
void EPWM3_Initialize(uint8_t freq) {
    ...
    ECCP3ASbits.PSS3AC = 0;

    ECCP3ASbits.PSS3BD = 0;

    PWM3CONbits.P3RSEN = 0;



    CCP3CONbits.P3M = 0b10;
    ...
    EPWM3_engine_config(0x00, 50);

    PWM3CONbits.P3DC = 0x50;
    ...
    TRISCbits.TRISC2 = 0;
    EPWM3_outputs_off();
    STATUS_Eng_V.status = 0;
    STATUS_Eng_V.turn = 0x00;
    return;
}
...
void EPWM3_period(uint8_t freq) {
    ...
    if (freq_lure != freq) {
        if (freq <= 4)
            TMR4_LoadPeriodRegister(((64000000 / 1000) / (4 * 4 * 16)) - 1);
        else
            TMR4_LoadPeriodRegister(((64000000 / 1000) / (freq * 4 * 16)) - 1);
        freq_lure = freq;
...
float EPWM3_duty_const(uint8_t freq) {
    ...
    if (freq != 0) {
        interval = (float) (64000000 / 1000) / (freq * 100 * 16);
...
void EPWM3_engine_config(uint8_t turn, uint8_t duty) {
    float interval = EPWM3_duty_const(0);
    uint16_t pulse_width_bit;
    if (turn == 0x01 || turn == 0x00) {
...
void EPWM3_engine_SoftStart(uint8_t turn, uint8_t dut_max) {
    if (STATUS_Eng_V.status == 0) {
        ...
        STATUS_Eng_V.status = 2;

        count_EPWM3_SStart = 50;
        EPWM3_engine_config(turn, count_EPWM3_SStart);
        if (TMR0_request_period() != 25)
            TMR0_period(25);
        else
            TMR0_Reload();
...
void EPWM3_engine_SoftStop(void) {
    if (STATUS_Eng_V.status == 1) {



        INTCON3bits.INT1IE = 0;
        STATUS_Eng_V.status = 3;
        if (TMR0_request_period() != 25)
            TMR0_period(25);
        else
            TMR0_Reload();
...
void EPWM3_STOP(void) {
    ...
    uint16_t pulse_width_bit;
    pulse_width_bit = 50 * interval_ISR;
