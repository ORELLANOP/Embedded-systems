...
void EPWM3_Initialize(uint8_t freq) {
    ...
    PSTR3CON = 0x13; /* Las salidas de ECCP2_PWM ocurren en el siguiente período 
                      * al primero del Timer. P3A y P3B se encuentran habilitados 
                      * para funcionar como salida PWM.
                      */
    CCPTMRS0bits.C3TSEL = 0b01; /* El módulo de PWM utiliza como base de tiempo a Timer 4 */
    if (freq < FREC_MIN)
...
void EPWM3_engine_SoftStart(uint8_t turn, uint8_t dut_max) {
    if (STATUS_Eng_V.status == INACTIVE_ENGINE) { // Si el motor está inactivo   
        /* Durante el arranque suave deshabilito la interrupción de protección contra 
         * sobrecorriente
         */
        ...
        TMR0_StopTimer();
        /* Reconfiguro la interrupción por protección de sobrecorriente
         */
        INTCON3bits.INT1IF = 0; // Borro el flag de INT1
        INTCON3bits.INT1IE = 1; // Habilito la interrupción de protección contra sobrecorriente
        INTCONbits.TMR0IF = 0;
    }
    return;
}
...
void EPWM3_engine_SoftStop(void) {
    if (STATUS_Eng_V.status == ACTIVE_ENGINE) { // Si el motor está en marcha...
        /* Durante el arranque suave deshabilito la interrupción de protección contra 
         * sobrecorriente
         */
        INTCON3bits.INT1IE = 0;
        STATUS_Eng_V.status = STOPPING_ENGINE; // motor en proceso de detención
...
void EPWM3_STOP(void) {
    /* Generación del shutdown del motor de elevación por interrupción del comparador
     * que mide sobrecorriente. Se pone al duty al 50% para hacer un frenado eléctrico.
